...
CREATE TABLE COMPLEJO_DEP(
    ...
    CONSTRAINT FK_COMP_SEDE FOREIGN KEY (id_sede) REFERENCES SEDE_OLIMPICA (id_sede),
    CONSTRAINT FK_COMP_JEFE FOREIGN KEY (id_jefe) REFERENCES JEFE_ORGANIZACION (id_jefe)
);
...
CREATE TABLE COMP_POLIDEPORTIVO (
    ...
    tipo_complejo VARCHAR2(20) DEFAULT 'POLIDEPORTIVO' NOT NULL,
    CONSTRAINT PK_COMP_POLIDEPORTIVO PRIMARY KEY (id_complejo),
    CONSTRAINT FK_COMP_POLIDEPORTIVO FOREIGN KEY (id_complejo) REFERENCES COMPLEJO_DEP (id_complejo)
);

CREATE TABLE COMP_DEPORTIVO_UNIC (
    id_complejo NUMBER(5),
    id_deporte NUMBER(3),
    tipo_complejo VARCHAR2(20) DEFAULT 'DEPORTIVO UNIC' NOT NULL,
    CONSTRAINT PK_COMP_DEPORTIVO_UNIC PRIMARY KEY (id_complejo),
    CONSTRAINT FK_COMP_DEPORTIVO_UNIC FOREIGN KEY (id_complejo) REFERENCES COMPLEJO_DEP (id_complejo),
    CONSTRAINT FK_COMP_DEPORTIVO_UNIC_DEPORTE FOREIGN KEY (id_deporte) REFERENCES DEPORTE (id_deporte)
);
...
CREATE TABLE AREA_DEPORTE(
    ...
    CONSTRAINT FK_AREA_COMP FOREIGN KEY (id_complejo) REFERENCES COMPLEJO_DEP(id_complejo),
    CONSTRAINT FK_AREA_DEP FOREIGN KEY (id_deporte) REFERENCES DEPORTE(id_deporte)
);
...
CREATE TABLE EVENTO (
    ...
    CONSTRAINT FK_EVENTO_COMP FOREIGN KEY (id_complejo) REFERENCES COMPLEJO_DEP(id_complejo)
);
...
CREATE TABLE COMISARIO (
    ...
    CONSTRAINT FK_COMISARIO_EVENTO FOREIGN KEY (id_evento) REFERENCES EVENTO(id_evento)
);
...
CREATE TABLE COMISARIO_EVENTO (
    ...
    CONSTRAINT FK_COM_ID_EVENTO FOREIGN KEY (id_comisario) REFERENCES COMISARIO(id_comisario),
    CONSTRAINT FK_COM_EVENTO_ID FOREIGN KEY (id_evento) REFERENCES EVENTO(id_evento)
);
...
CREATE TABLE EQUIPAMIENTO (
    ...
    CONSTRAINT FK_EQUIPAMIENTO_EVENTO FOREIGN KEY (id_evento) REFERENCES EVENTO(id_evento)
);

CREATE TABLE EQUIPAMIENTO_EVENTO (
    id_equipamiento NUMBER(5),
    id_evento NUMBER(5),
    CONSTRAINT FK_EQUIP_EVENTO_ID FOREIGN KEY (id_equipamiento) REFERENCES EQUIPAMIENTO(id_equipamiento),
    CONSTRAINT FK_EQUIPAMIENTO_EVENTO_ID FOREIGN KEY (id_evento) REFERENCES EVENTO(id_evento)
);